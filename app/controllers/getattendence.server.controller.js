...
exports.list = function(req, res) {
            ...
            // console.log('endday1',endday1);
             ...
             var endday=lastday.toISOString().slice(0,10)
			 console.log('startday',startday);
			 console.log('endday',endday);
     		 Punchin.find({userid:mongoose.Types.ObjectId(userid),date:{$gte:startday,$lte:endday}},{date:1,status:1,timein:1,_id:0,timeout:1}, function(err, result) {
     		 			//console.log("helolo");
		  		...
		  		if (err){
		   		throw err;
					...
					console.log('result1',result1);
					 Leave.find({userid:mongoose.Types.ObjectId(userid),leavestauts:'grant',date:{$gte:startday,$lte:endday}},{date:1,status:1,_id:0}, function(err, result) {
					 	if(err){
					 		throw err;
					 	}else{
					 		var result2=result;
